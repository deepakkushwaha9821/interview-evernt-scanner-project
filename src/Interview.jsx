    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(
          `${API_BASE}/proctor/latest/${pairCode}?t=${Date.now()}`,
          { cache: "no-store" }
        );
        if (!res.ok) {
          setFeedStatus("Unable to fetch mobile feed status");
          return;
        }
        ...
        if (data.frame) {
          ...
          setFeedStatus("Live mobile feed");
        } else {
          setMobileFrame(null);
          setFeedStatus(data.connected ? "Mobile connected, waiting for first frame..." : "Mobile not linked");
        }
        if (data.detection) {
        ...
        setFeedStatus("Network issue while loading mobile feed");
